...
public class ResultViewer {
    ...
    public void showChart(Map<Integer, StdCluster> clusters) {
        DefaultXYDataset xyDataset = new DefaultXYDataset();

        for (Map.Entry<Integer, StdCluster> entry : clusters.entrySet()) {
            int label = entry.getKey();
            StdCluster cluster = entry.getValue();
            int size = entry.getValue().getGrids().size();
            double[][] data = new double[2][size];
            for (int i = 0; i < size; i++) {
                StdGrid grid = cluster.getGrids().get(i);
